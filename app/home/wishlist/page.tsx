...
export default function WishlistPage() {
  ...
  const handleRemoveFromWishlist = async (coinId: string) => {
    try{
      const token = user?.jwt;
      const response = await axios.patch(`http://localhost:8080/api/wishlist/add/${coinId}`, {}, {
        headers: {
          Authorization: `Bearer ${token}`,
        }
      });
      console.log("Coin remove successfully");
      // Remove the coin from wishlistCoins state
      setWishlistCoins(prevCoins => prevCoins.filter(coin => coin.id !== coinId));
    }catch(err){
